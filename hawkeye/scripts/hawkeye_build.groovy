def projectName="hawkeye"
node {
    stage("Clone and checkout source code for $projectName") {
        git branch: "${DEPLOYMENT_BRANCH}", credentialsId: "STASH_CREDENTIALS",
                url: "ssh://git@stash.internal.macquarie.com/bfssds/hawkeye.git"
    }

    stage("Gradle build") {
      sh """
        ${GRADLE5_HOME}/bin/gradle clean build tag-build -Prelease=true -Dorg.gradle.java.home=\${JAVA11_HOME}
      """
    }
