def projectName="chromia-api"
node {
    stage("Clone and checkout source code for $projectName") {
        git branch: "${DEPLOYMENT_BRANCH}", credentialsId: "STASH_CREDENTIALS",
                url: "ssh://git@stash.internal.macquarie.com/SDSCQL/chromia-api.git"
    }

    stage("Gradle build") {
      sh """
        ${GRADLE5_HOME}/bin/gradle build-info clean build -Pbuild.number=$BUILD_NUMBER -Dorg.gradle.java.home=\${JAVA11_HOME}
      """
    }
